...
public class AddPicJokesActivity extends BaseActivity implements View.OnClickListener {
    public static final String TAG = "AddPicJokesActivity";
    ...
    public int getLayoutId() {
        return R.layout.activity_add_pic_jokes;
    }
    ...
    private void getData() {
        ...
        myOkHttp.post()
                ...
                .params(params)
                .tag(this)
                .enqueue(new GsonResponseHandler<JokeImgModel>() {
                    @Override
    ...
    //分析数据
    private void analyseData(JokeImgModel response) {
        new AsyncTask<JokeImgModel, Void, Integer>() {

            @Override
            protected Integer doInBackground(JokeImgModel... jokeImgModels) {
                if ( jokeImgModels[0].getResult() != null ) {
                    //获取本地数据库中类型为2的所有数据
    ...
    class ListAdapter extends BaseQuickAdapter<JokeModel, BaseViewHolder> {
        ...
        protected void convert(final BaseViewHolder helper, final JokeModel item) {
            ...
            helper.setText(R.id.richText1, item.getImageUrl());
        }
    }
}
